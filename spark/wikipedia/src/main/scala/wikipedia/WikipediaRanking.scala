...
object WikipediaRanking extends WikipediaRankingInterface {
   ...
   */
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.aggregate(0)((acc, text) => acc + (if (text.mentionsLanguage(lang)) 1 else 0), _ + _)
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = langs.map(lang =>
    lang -> occurrencesOfLang(lang, rdd)).sortBy(_._2)(Ordering[Int].reverse)
