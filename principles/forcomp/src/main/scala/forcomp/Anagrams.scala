...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase().toList.groupBy(c => c).view.mapValues(_.size).toList.sorted
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    x ++ y.map(z => (z._1, -z._2)).groupBy(x => x._1).view.mapValues(_.map(_._2).sum).toList.filterNot(_._2 == 0).sorted
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceOccurrencesAnagrams(occurrences: Occurrences): List[Sentence] = occurrences match {
      case Nil => List(Nil)
      case _   => for {
        comb <- combinations(occurrences)
        word <- dictionaryByOccurrences(comb)
        restSentence <- sentenceOccurrencesAnagrams(subtract(occurrences, comb))
