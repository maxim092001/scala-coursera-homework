...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    neighbors.filterNot(f => explored.contains(f._1))
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val nextPaths = for {
        (blk, path) <- initial
        (nextBlock, nextPath) <- newNeighborsOnly(neighborsWithHistory(blk, path), explored)
      } yield (nextBlock, nextPath)
      val newExplored = (nextPaths map (_._1)).toSet
      nextPaths #::: from(nextPaths, explored.union(newExplored))
    }
   ...
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(List((startBlock, Nil)).to(LazyList), Set(startBlock))
