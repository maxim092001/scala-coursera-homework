...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] = Decoder.fromPartialFunction(
    { case Json.Num(value) if value.isValidInt => value.intValue })
  ...
  implicit val booleanDecoder: Decoder[Boolean] = Decoder.fromPartialFunction(
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromPartialFunction(
      { case Json.Arr(arr) => arr.map(f =>
        f.decodeAs(decoder).get
      )
      }
    )
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromPartialFunction(
      { case Json.Obj(value) => value(name).decodeAs(decoder).get }
    )
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =
    Decoder.fromPartialFunction({ case Json.Obj(value) =>
      Person(value("name").decodeAs(implicitly[Decoder[String]]).get, value("age").decodeAs(implicitly[Decoder[Int]]).get)
    })
...
trait ContactsCodecs {
  ...
  implicit val contactsDecoder: Decoder[Contacts] =
    Decoder.fromPartialFunction({ case Json.Obj(value) =>
      Contacts(value("people").decodeAs(implicitly[Decoder[List[Person]]]).get)
    })
